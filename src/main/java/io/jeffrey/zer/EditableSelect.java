...
public class EditableSelect {
  ...
  /**
   * @param parent
   *          the parent that needs to be updated once a selection is made
   * @param data
   *          what we are selecting from
   */
  ...
  public HBox createLinkedControlBox() {
    ...
    All.setOnAction(dontcare -> set(null));
    hbox.getChildren().add(All);

    if (priorId != null) {
      final Button Previous = new Button(priorId);
      Previous.setOnAction(dontcare -> set(priorId));
...

    {
      ...
      Select.valueProperty().addListener((ChangeListener<String>) (val, before, after) -> {
        id = after;
        parent.sync();
      });
    ...
    if (nextId != null) {
      ...
      Next.setOnAction(dontcare -> set(nextId));
