...
public class PointChain implements Iterable<SelectablePoint2> {
	 ...
	 */
	public void describePossibleActionsBasedOnSelectedVertices(
			final List<String> actions, final boolean asLoop) {
	 ...
	 * @return an iterator over all the lines where a line is defined as a pair
	 *         of points
	 */
	public Iterable<SelectablePoint2[]> lines(final boolean asLoop) {
		...
		return new Iterable<SelectablePoint2[]>() {
			...
			public Iterator<SelectablePoint2[]> iterator() {
				...
				for (int k = 0; k < sz; k++) {
					final SelectablePoint2 point = points
							.get(k % points.size());
					point.cachedIndex = k;
					final SelectablePoint2 next = points.get((k + 1)
							% points.size());
	...
	/**
	 * Helper: convert the given raw points serialize form into an array of
	 * points
	 */
	...
	public Iterable<SelectablePoint2[]> selectedSegments(final boolean asLoop,
		...
		return new Iterable<SelectablePoint2[]>() {
			...
			public Iterator<SelectablePoint2[]> iterator() {
				...
				for (int k = 0; k < sz; k++) {
					final SelectablePoint2 point = points.get((k + offset)
							% points.size());
					final SelectablePoint2 next = points.get((k + offset + 1)
							% points.size());
