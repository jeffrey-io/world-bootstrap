...
public abstract class AbstractMapEditor<T extends AbstractMapEditorItemRequirements> implements Syncable {
  ...
  protected AbstractMapEditor(final Map<String, T> things, final String id, final Syncable parent) {
    ...
    selector.valueProperty().addListener((ChangeListener<String>) (val, before, after) -> {
      if (skipNotification) {
        return;
      }
      notifyUpdate(after);
    ...
    selector.focusedProperty().addListener((ChangeListener<Boolean>) (dontCare, reallyDontCare, after) -> {
      if (after) {
